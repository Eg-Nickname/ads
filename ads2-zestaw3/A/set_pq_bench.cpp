...
constexpr std::size_t MAX_N = 50000;
constexpr std::size_t STEP = 5000;
...
void run_benchmarks() {
    std::ofstream csv("set_pq_benchmarks.csv");
    // Header for the CSV
    csv << "N,Insert_Avg_ms,Remove_ms\n";

    auto idnty = [](const uint32_t& e) { return e; };

    std::vector<SetPq<uint32_t, decltype(idnty)>> pqs(
        NUM_RUNS, SetPq<uint32_t, decltype(idnty)>(idnty));
